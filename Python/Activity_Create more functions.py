...
# we observe that even though the list is organized in chronological order of month, after using the sorted() inbuild function to print the output, the output is printed in increaseing order of the numbers in the list.
...
# Using the max() function , the highest number in the listed is printed.
...
# The output displays the username and current day logins for ejones which is passed as an argument while calling the function. The output will vary for different users.
...
# The function calculates the login ration and return the values, later that value is stored in a variable and that prints the login analysis
