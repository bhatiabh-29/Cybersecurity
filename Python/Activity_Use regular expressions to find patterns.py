...
# r15\w+ pattern was used for the traget pattern and if each component like "\" or "w" an empty list would be printed and if "+" symbol is missing it would display just one letter or digit after r15.
...
# All the ip address gets extracted but not all of them have one and three digits in every segment.
...
# Only valid Ip addresses are extracted with the pattern "\d\d\d\.\d\d\d\.\d\d\d\.\d\d\d"
...
# Regular expressions are used to search, match, and manipulate text patterns in strings. They are powerful tools for security analyst to analyze logs and search thorugh huge log files or manipulate data.
